...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $products = $productController->filterProducts($categoryName);
} else {
    $products = $productController->listProductsByCategory($categoryName);
}
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <script>
        function validateForm() {
            ...
            if (isNaN(minPrice) || isNaN(maxPrice) || minPrice <= 0 || maxPrice <= 0) {
                errorMessage.textContent = "Both price fields must be filled and greater than 0.";
                return false;
            }
            if (minPrice > maxPrice) {
                errorMessage.textContent = "Minimum price cannot be greater than maximum price.";
                return false;
            }
